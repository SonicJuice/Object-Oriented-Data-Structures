...
class LinkedList(object):
    ...
    """ inserts a node at the specified index; if it's > '__size', raise an exception. If it's == '__size', call 'addNode' to add it to the end. If it's 0, 
    insert 'new_node' at the beginning of by updating the 'next_node' reference of 'new_node' to the current '__head', the 'prev_node' reference of the current 'head' 
    to the 'new_node', and setting  '__head' to 'new_node.' If it isn't 0 nor == '__size', traverse the list to find the node at the specified index. Then, update the 
    references of 'new_node' and surrounding nodes to insert it correctly. If 'index = self.__size - 1', update '__tail' to the 'new_node,' 
    before incrementing '__size.'' """
    def insertNode(self, d, index):
