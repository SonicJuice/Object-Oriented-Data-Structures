...
class Graph:
    ...
    def addNode(self, new_node):
        " if the new node isn't yet in the AL, add the new node as a key in '__nodes' w/ an empty dictionary as its value, representing no outgoing edges from the new node. """
    ...
    def addEdge(self, origin, destination, weight):
        ...
        """ if either node doesn't exist, add it to the AL as empty dictionaries. If the weight is numeric and there's no pre-existing edge, add a new key-value pair to '__nodes'.  """
    ...
    def depthFirst(self, node, visited = None):
        """ utilises recursive calls to traverse a graph from the current node, going as far down as possible before backtracking. The given node is added to 'visited'; if the neighboring node hasn't been visited, pass it as an argument into the recursion. This continues until all nodes reachable from the starting node have been visited. """
    ...
    def breadthFirst(self, node):
        """ utilises a queue to explore nodes at the same depth before ascending. The given node is added to 'queue' and 'visited', before a loop is entered until 'queue' is empty. In each iteration, dequeue the front node and explore its neighbours; for each, if it hasn't been visited, enqueue it to the back and add it to 'visited'. This continues until all nodes reachable from the starting node have been visited. """
        if node not in self.__nodes:
            raise ValueError("Node doesn't exist.")
    ...
    def dijkstrasShortestPath(self, node):
        """ for this method, add edges in the form ("A", "B"), ("B", "A"). Here, a dictionary is initialised with the source as the key and its distance from itself as 0; all other node distances are set to infinity. """
        if node not in self.__nodes:
            raise ValueError("Node doesn't exist.")
        ...
        while pq:
            """ a heap is used to implement a priority queue; 'heappop' removes and returns the smallest element, whilst the order is adjusted to maintain the order properties (see 'Priority Queue Object.py'). """
            distance, current_node = heappop(pq)
            if current_node in visited:
                continue
            visited[current_node] = True

            """ update the distances to the neighboring nodes of 'current_node' if a shorter path is found. For each neighboring node, calculate the distance to 'neighbour' through 'current_node'. """
            for neighbour, weight in self.__nodes[current_node].edges.items():
                """ achieved by adding the distance from 'current_node' to 'neighbour' to the distance from the source node to 'current_node'. If this is < the current distance to neighbour in 'distances', update the distance and push (new_distance, neighbour). """
